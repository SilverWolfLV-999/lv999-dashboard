...
4. 每张图用 createImageAsset 生成（prompt 自包含：商品主体、场景、构图、色调、文字内容与位置）；详情页文案用 createAsset 存为 markdown。
...
- 封面：竖版 3:4，画面主体突出，需要标题文字时写明内容、位置与风格。
...
3. 封面用 createImageAsset 生成（aspect 传 "3:4"，prompt 自包含描述画面与标题文字）；用户已有满意的图想改封时用 editImageAsset。
